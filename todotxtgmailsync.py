...
SETUP AND CONFIGURATION:
        -Download and install the Python Google APIs Client Library 
...
def usage():
    """Prints progam usage"""
    print "USAGE: %s [pull|push|push all]"% (sys.argv[1], )
...
def addToDone(task):
    ...
    todo = "x " + date.strftime("%Y-%m-%d") + " " + re.sub("\([A-Z]\) ","",task["title"],1)
    ...
    for tasklist in tasklists["items"]:
        ...
        for task in tasks["items"]:
            ...
            if task["title"].strip():
                ...
                count = 1
                for item in dones:
                    if re.sub("\([A-Z]\) ","",task["title"].strip(),1) in item:
                        foundInDone = count
                        break
                    count+=1            
...
def push(filename):
    ...
    try:
        todoFile = codecs.open(os.environ.get("TODO_DIR")+"/"+filename+".txt", "r",
                encoding='utf-8')
